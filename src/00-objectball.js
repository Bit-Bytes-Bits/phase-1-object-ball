...
function teamNames() {
  const nameOfTeams = gameObject();
  const arrayTeamNames = [];
  for (let name in nameOfTeams) {
    arrayTeamNames.push(nameOfTeams[name]["teamName"]);
  }
  return arrayTeamNames;
}

function playerNumbers(teamName) {
  let game = gameObject();
  const jerseyNumbers = [];
  for (let homeAndAway in game) {
    if (teamName === game[homeAndAway]["teamName"]) {
      let playersObj = game[homeAndAway]["players"];
      for (let playerKey in playersObj) {
        jerseyNumbers.push(playersObj[playerKey]["number"]);
      }
    }
  }
